...
# Helper
function Show-Help {
    Write-Host "Ralph Loop for Google Gemini CLI (PowerShell)"
    Write-Host "Usage: .\scripts\ralph-loop-gemini.ps1 [iterations] [plan]"
}
...
$SESSION_LOG = Join-Path $LOG_DIR "ralph_gemini_${MODE}_session_${timestamp_log}.log"
...
try {
    ...
    while ($true) {
        ...
        $LOG_FILE = Join-Path $LOG_DIR "ralph_gemini_${MODE}_iter_${ITERATION}_${ts_file}.log"

        $GEMINI_FLAGS = @()
        if ($YOLO_ENABLED) { $GEMINI_FLAGS += "--yolo" }

        Write-Host "${PURPLE}LOOP $ITERATION${NC}"

        # FIX: Pipe input to Gemini instead of passing as argument
        $GEMINI_OUTPUT = $promptContent | & $GEMINI_CMD $GEMINI_FLAGS 2>&1 | Tee-Object -FilePath $LOG_FILE  

        if ($GEMINI_OUTPUT -match "<promise>(ALL_)?DONE</promise>") {
