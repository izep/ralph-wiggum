...
function Record-LoopResult {
    ...
    if ($newState -ne $currentState) {
        switch ($newState) {
            $CB_STATE_OPEN { Write-Host ("{0}🚨 CIRCUIT BREAKER OPENED: {1}{2}" -f $RED, $reason, $NC) }
            $CB_STATE_HALF_OPEN { Write-Host ("{0}⚠️  CIRCUIT BREAKER: Monitoring - {1}{2}" -f $YELLOW, $reason, $NC) }
            $CB_STATE_CLOSED { Write-Host ("{0}✅ CIRCUIT BREAKER: Normal Operation - {1}{2}" -f $GREEN, $reason, $NC) }
        }
    }
...
function Reset-CircuitBreaker {
    ...
    $state | ConvertTo-Json | Set-Content $CB_STATE_FILE
    Write-Host ("{0}✅ Circuit breaker reset to CLOSED state{1}" -f $GREEN, $NC)
}
