...
# Helper
function Show-Help {
    Write-Host "Ralph Loop for OpenAI Codex CLI (PowerShell)"
    Write-Host "Usage: .\scripts\ralph-loop-codex.ps1 [iterations] [plan]"
}
...
$SESSION_LOG = Join-Path $LOG_DIR "ralph_codex_${MODE}_session_${timestamp_log}.log"
...
try {
    ...
    while ($true) {
        ...
        $LOG_FILE = Join-Path $LOG_DIR "ralph_codex_${MODE}_iter_${ITERATION}_${ts_file}.log"
        $OUTPUT_FILE = Join-Path $LOG_DIR "ralph_codex_output_iter_${ITERATION}_${ts_file}.txt"

        $CODEX_FLAGS = @("exec")
        if ($YOLO_ENABLED) { $CODEX_FLAGS += "--dangerously-bypass-approvals-and-sandbox" }

        Write-Host "${PURPLE}LOOP $ITERATION${NC}"

        # Run Codex
        $promptContent | & $CODEX_CMD $CODEX_FLAGS - --output-last-message $OUTPUT_FILE 2>&1 | Tee-Object -FilePath $LOG_FILE

        $CODEX_OUTPUT = if (Test-Path $OUTPUT_FILE) { Get-Content $OUTPUT_FILE -Raw } else { Get-Content $LOG_FILE -Raw }

        if ($CODEX_OUTPUT -match "<promise>(ALL_)?DONE</promise>") {
